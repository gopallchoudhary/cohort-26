...
const SOCKET_URL =
	import.meta.env.VITE_SOCKET_URL || "https://cohort-26-1.onrender.com/";
...
const SketchGrid = () => (
	...
	>
		{/* vertical line 1 */}
		<path
			d="M110,8 Q111,80 109,160 Q110,240 111,322"
			stroke="#2c2c2c"
			strokeWidth="3.5"
			strokeLinecap="round"
			fill="none"
		...
		{/* vertical line 2 */}
		<path
			d="M220,8 Q221,90 219,165 Q220,245 221,322"
			stroke="#2c2c2c"
			strokeWidth="3.5"
			strokeLinecap="round"
			fill="none"
		...
		{/* horizontal line 1 */}
		<path
			d="M8,110 Q90,109 165,111 Q245,110 322,109"
			stroke="#2c2c2c"
			strokeWidth="3.5"
			strokeLinecap="round"
			fill="none"
		...
		{/* horizontal line 2 */}
		<path
			d="M8,220 Q90,221 165,219 Q245,220 322,221"
			stroke="#2c2c2c"
			strokeWidth="3.5"
			strokeLinecap="round"
			fill="none"
...
function App() {
	...
	const [roomStatus, setRoomStatus] = useState<
		"lobby" | "waiting" | "active" | "disconnected"
	>("lobby");
	...
	useEffect(() => {
		if (gameMode === "online") {
			...
			newSocket.on(
				"room-created",
				({ code, symbol, scores: initialScores }) => {
					setRoomCode(code);
			...
			newSocket.on(
				"opponent-joined",
				({ board: sBoard, currentPlayer: sTurn, scores: sScores }) => {
					setRoomStatus("active");
					setBoard(sBoard);
					setCurrentPlayer(sTurn);
					...
					<div className="lobby-container">
						<div className="lobby-card">
							...
							<p className="lobby-desc">
								Start a new online game and invite a friend.
							</p>
				...
				{gameMode === "online" && roomStatus === "disconnected" && (
					<div className="lobby-container">
						<div className="lobby-card disconnected-card">
							...
							<p className="lobby-desc">
								Your opponent has disconnected from the room.
							</p>
				...
				{(gameMode === "local" || roomStatus === "active") && (
					<>
						...
						{gameMode === "online" &&
							...
							playerSymbol && (
								<div
									className={`player-badge badge-${playerSymbol.toLowerCase()}`}
								>
